...
-- 1. First, disable RLS on project_members to stop recursion
...
DROP POLICY IF EXISTS "Users can update project members for projects they are members of" ON project_members;
DROP POLICY IF EXISTS "Users can delete project members for projects they are members of" ON project_members;

-- 3. Drop the problematic function
DROP FUNCTION IF EXISTS get_projects_for_user(UUID);

-- 4. Create a new function that handles the enum types correctly
...
-- 5. Ensure project_members table exists with correct structure
...
-- 7. Add current user to projects that don't have members
...
-- 8. Update existing projects to have default values
UPDATE projects 
SET status = 'on_going' 
